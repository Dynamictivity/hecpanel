...
class PosixgroupSchemasController extends PosixAppController {
    ...
    function edit($id) {
        if (!empty($this->request->data)) {
            if (isset($this->request->data['PosixgroupSchema']['dn']) && !empty($this->request->data['PosixgroupSchema']['dn'])) {
                $id = $this->request->data['PosixgroupSchema']['dn'];
            }
            $this->PosixgroupSchema->id = $id;
            unset($this->request->data['PosixgroupSchema']['nonmemberuid']);
            $this->log("What I'm Going to save" . print_r($this->request->data['PosixgroupSchema'], true) . "\nFor the following ID:" . $this->PosixgroupSchema->id, 'debug');
            $result = $this->PosixgroupSchema->save($this->request->data);
            if ($result != false) {
        ...
        $options['targetDn'] = $id;
        $options['scope'] = 'base';
        $this->request->data = $this->PosixgroupSchema->find('first', $options);
        ...
        $this->log("Group for $id:" . print_r($group, true) . "\nThis group looks like:" . print_r($this->request->data, true), 'debug');
