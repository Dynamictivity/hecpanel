...
class LdapAuthenticate extends BaseAuthenticate {
    ...
    public function __construct(ComponentCollection $collection, $settings = array()) {
        ...
        if (isset($this->sqlUserModel)) {
            $this->form = $this->sqlUserModel;
        }
    ...
    public function authenticate(CakeRequest $request, CakeResponse $response) {
        ...
        return $this->_findLdapUser(
                        $dn, $request->data[$this->form][$fields['password']]
        );
    }
    ...
    public function _getDn($attr, $query) {
    ...
    public function existsOrCreateSQLGroup($user = null, $groups = null) {
        if (!$user || !$groups)
            return false;
        ...
        foreach ($groups as $groupName => $dn) {
            ...
            if ($sqlGroup = $this->sqlGroupModel->find('first', array('conditions' => array('Group.name' => $groupName)))) {
                ...
                if (isset($user['username']) && is_string($user['username']) && isset($sqlGroup[$this->sqlUserModel->alias]) && is_array($sqlGroup[$this->sqlUserModel->alias])) {
    ...
    public function existsOrCreateSQLUser($user) {
        ...
        if (isset($user[$this->model->alias][$userPK]))
            $username = $user[$this->model->alias][$userPK];
        if (is_array($username) && isset($username[0]) && !is_array($username[0]))
            $username = $username[0];
        ...
        if (isset($result) && !empty($result)) {
            ...
            if (isset($user[$this->model->alias]['displayname']) && !empty($user[$this->model->alias]['displayname']))
                $u['displayname'] = $user[$this->model->alias]['displayname'];
            if (isset($user[$this->model->alias]['dn']) && !empty($user[$this->model->alias]['dn']))
                $u['dn'] = $user[$this->model->alias]['dn'];
            if (isset($user[$this->model->alias]['cn']) && !empty($user[$this->model->alias]['cn'])) {
                $nameArray = explode(' ', $user[$this->model->alias]['cn']);
                $u['first_name'] = array_shift($nameArray);
                $u['last_name'] = implode(' ', $nameArray);
            }
            if (isset($username) && !empty($username))
                $u['username'] = $username;
            if (isset($user[$this->model->alias]['mail']) && !empty($user[$this->model->alias]['mail']))
                $u['email'] = $user[$this->model->alias]['mail'];
